...
if [[ ! -f "$ENV_FILE" ]]; then
  echo "Missing $ENV_FILE. Create it first (copy from .env.prod.example)."
  exit 1
fi
...
if [[ -z "${NEXT_PUBLIC_SITE_URL:-}" || -z "${NEXT_PUBLIC_API_BASE_URL:-}" ]]; then
  echo "NEXT_PUBLIC_SITE_URL and NEXT_PUBLIC_API_BASE_URL must be set in $ENV_FILE"
  exit 1
fi
...
echo "Starting database/cache first"
docker compose "${COMPOSE_ARGS[@]}" up -d --build postgres redis
...
echo "Running backend migrations/checks"
docker compose "${COMPOSE_ARGS[@]}" exec -T api python manage.py migrate --noinput
docker compose "${COMPOSE_ARGS[@]}" exec -T api python manage.py collectstatic --noinput
